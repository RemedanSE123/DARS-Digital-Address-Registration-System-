...
$mendersResult = $conn->query($mendersSql);
              ...
              <th>recorded time</th>
                  ...
                  while ($menderRow = $mendersResult->fetch_assoc()) {
                    echo '<tr>
                            <td>'.$menderRow["mender_id"].'</td>
                            <td><a href="displayhouseno.php?mender_id='.$menderRow["mender_id"].'">'.$menderRow["mender_name"].'</a></td>
                            <td><img src="'.$menderRow["mender_map"].'" alt="Mender Map" class="image-width"></td>
                            <td>'.$menderRow["description"].'</td>
                            <td>'.$menderRow["recorded_by_full_name"].'</td>
                        <td>'.$menderRow["recorded_time"].'</td>
                            <td>
                              <a href="editmender.php?mender_id='.$menderRow["mender_id"].'" class="edit-btn" title="Edit"><i class="fas fa-edit"></i></a>
                              <a href="delete/deletemender.php?mender_id='.$menderRow['mender_id'].'&kebela_id='.$kebelaId.'&worda_id='.$wordaId.'&region_id='.$regionId.'" class="delete-btn" name="delete" onclick="return confirmAction(\'delete\')" title="Delete"><i class="fas fa-trash"></i></a>
                            </td>
                          </tr>';
                  }
  ...
  function confirmAction(action) {
    ...
    if (action === 'delete') {
      confirmationMessage = 'Are you sure you want to delete this Mender?';
    } 
